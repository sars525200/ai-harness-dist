...
> ⚠ **W-5 的字面於 2026-08-23 被票 10 推翻**（`.scratch/skill-watch-multiplatform/issues/10-*.md`；
> **2026-09-04 訂正**：原文誤寫「票 06」，票號與 `:916`「三種初次體驗的預設」相撞——
> 那支才是票 06。基準搬家／v3 遷移是票 10，見 `:923`）：
...
>
> ✅ **搬家於 2026-09-04 實作**（`tools/migrate_skill_baselines.py`，一次性·冪等）。
> ⚠ **只做了搬家這一半，票 10 的 v3 巢狀遷移仍是凍結狀態**（§17 那張表沒有動）。
> 三件實作時才浮出來、票文沒寫的事：
> - **`officialCrossCheck` 一起搬**。票文只點名 `baselines`，但它記的是「上次跟官方
>   文件比的差集」，同樣是本機快照；留在版控裡，別部門拿到的是我這台的
>   `missingLocally`，而 R3-2 的判定看的是「鍵在不在」⇒ **鍵在但內容是別人的**
>   正好落進那條註解描述的洞。
> - **`--capture` 以前一開頭就 `load_doc()`，檔不在就拒跑**。搬家前基準在版控裡、
>   一定存在，所以這條路從來沒被走過；搬出去之後它是建立基準的唯一入口，
>   新機第一次跑必定拒跑、永遠建不了基準。已改成只有 `--capture` 容許檔不存在
>   （`--show`／`--compare` 維持拒跑——沒有基準不等於沒有變動）。
> - **`skill_inventory.build()` 的 `baselines` 來源要跟著拆成第二個參數**。留著
>   `doc.get("baselines")` 的話它會回空 dict、迴圈空轉、不報錯，`skills[].modes`
>   悄悄變空——§14 早就點名這是「改前先 grep 找齊全部 copy」要擋的第一個坑。
>   `tests/test_skill_inventory_write_gate.py` 補了兩條守它，樣本刻意把 `baselines`
>   放在清冊那份當誘餌：讀錯來源會綠，所以那兩條才釘得住。
...
### 4.3 實測翻出的新分岔 W-8（**已定案**：兩份基準各比各的，見下方決定）
...
⚠ **2026-09-04 補：R2-3 的處置沒回寫進這裡（規格缺口，只補文件不改行號）**。
Round 2 #3（見 §16.2）指出「17 處」仍然太窄——那是同一種 grep 樣式漏掉的問題，
`tools/skill_watch_run.py` 裡「一維 mode＝`headless`」被**硬寫死在好幾處**，不只
`officialCrossCheck` 那三行：`sanity_check()` 呼叫、`compare()` 呼叫、`capture()` 呼叫、
以及至少一處報告文字寫死「headless／中性目錄」。
⚠ **R2-3 本身列的行號（`:393`／`:400`／`:435`／`:459`／`:487`）已經是舊版行號，不要照抄**——
2026-09-04 複查該檔現況是 636 行，`sanity_check` 在 `:499`、`compare` 在 `:559`、
`capture` 在 `:615`，另有多處字面 `"headless"`（如 `:180`）；改動前一律重新
`grep -n "sanity_check\|compare(\|\.capture(\|\"headless\"" tools/skill_watch_run.py` 抓當下真實行號，
不要沿用計畫書任何一版寫死的數字（這正是 W9 反覆踩到的同一個病）。
...
⚠ **2026-09-04 補：R2-4 的處置沒回寫進這裡**。Round 2 #12 原本只改了 `capture`，
但覆核發現**本機自建過濾（`filterLocal`）現在有兩處各管各的**：`skill_watch.py` 一處，
`tools/skill_watch_run.py` 另一處（後者比前者先執行，所以就算前者接了 `local_names()`，
`skill_watch_run.py` 那邊送進去的清單早就先被削過一次，數量守衛看到的是削過的假數字，
等於 `filterLocal:false` 形同虛設）。**接受的處置**：兩處都要改吃這裡定義的
`local_names()`（不是各自維持一份寫死的本機清單）。⚠ 兩處的現況行號未經 2026-09-04 複查，
施作前先 `grep -n "local_name\|filterLocal" tools/skill_watch.py tools/skill_watch_run.py` 抓當下行號，
不要沿用票面舊數字。
...
⚠ **2026-09-04 補：R2-7 的處置沒回寫進這裡**。Round 2 #7 指出 14.4⑦「分辨這是停用 N 天後
的重新啟用」這個要求，兩份設定檔（`state/skill_watch_platforms.json`、心跳檔）**都沒有時間戳**
可以拿來判斷——唯一候選是心跳，但它整份覆寫（不含平台維度的合併），拿基準 `capturedAt` 當代理
又會被「穩定平台的日期停在幾個月前」誤判成長期停用，導向 bootstrap 把好基準覆蓋掉。
**接受的處置**：`state/skill_watch_platforms.json` 要加 `disabledAt`／`lastEnabledAt` 兩個欄位；
心跳的 per-platform 欄位要**合併寫入**、不是整份覆寫（現況只有 `lastSuccessAt` 做了合併，
是特例不是通例，其餘欄位要跟進）。
...
- ⚠ **2026-09-04 補：R2-8 的處置沒回寫進這裡（先前只在 §14.6 Execute 清單的括號裡提過一句，
  本文沒有）**。Round 2 #8 指出 `minPlausibleCount` 這道下限擋不住 bootstrap 把「一次壞擷取」
  固化——實跑：空 doc 配 25 個編造但合法的 kebab 名字會通過，5 個也通過，只有 4 個以下才被拒；
  而該參數的設計動機正是「讓能力少的平台建得了基準」，人遇到擋下時的直覺反應就是把它調低，
  等於把唯一那道門再放寬一次。**接受的處置**：`/skill-watch` 是手動指令、人本來就站在旁邊，
  bootstrap **必須印出擷取到的完整清單、問一次人確認才落檔**，不能無聲自動寫入。
...
- [ ] **前置工作項（Round 2 #5 升級）**：先給 `run.py` 做注入縫（`HEARTBEAT_PATH`／
      ...
      「先證明它會紅」在那五條上會整批失效
- [ ] Execute（**2026-09-04 訂正**：整條打勾方式沒反映拆分，實況是 3 支已完成、6 支凍結——
      對照見 §17 決策票表；凍結的都是「第二個平台接上去」才用得到的機器，本 effort 不做也不刪）：
  - [x] ①設定拆兩層（票 08）
  - [ ] ②adapter（票 09·❄ 凍結）
  - [ ] ③v3 遷移（票 10·❄ 凍結）
  - [ ] ④停用語意（票 11·❄ 凍結）
  - [x] ⑤勾選流程（票 12）
  - [ ] ⑥錯誤隔離與心跳（票 13·❄ 凍結）
  - [ ] ⑦bootstrap（票 14·❄ 凍結；**加人確認才落檔**·R2-8）
  - [ ] ⑧TODOS 列帶平台（票 15·❄ 凍結）
  - [x] ⑨初次體驗三情境（票 06·R2-9）
...
**⑦ 同日再抓到一個同型的漏，已修**（user 問「所以這個技能的回覆報表呢？」帶出來的）。

§18.7 ⑥ 那一列寫著「報告改版：一句結論＋對照表＋**可用／可剃除判斷**；Workflow 不再隱形」。
實查 `SKILL.md`：**`可剃除` 0 命中**，`Workflow` 只出現在步驟 2 的「怎麼讀工具輸出」那張表，
**步驟 3／4 的回報規格一個字都沒提** ⇒ **那次改版只落在工具的 stdout，沒有落進回報規格**。
工具乖乖印 `[Workflow] deep-research — …`，模型照 `SKILL.md` 寫回報時整個丟掉；
2026-08-24 02:28 那次無變動回報就是這樣丟的（我丟的）。

**與票 19、票 16 同一類病：規則沒有承載體**，差別只在缺口在文件那一側。

**修法**（commit `dca64f0`）：`SKILL.md` 新增 §3.5「這兩塊無論有沒有變動都要帶」
（Workflow 現況 ＋「官方有、本機沒有」那批的三欄可用／可剃除判斷）；
訂正步驟 4 的「不要端出來」——**該禁的是「當成變動」，不是「不准提」，原句只寫了前半**；
完成判準 +2 條；新增測試 `test_skillmd_report_spec_covers_what_the_tool_prints`，
判準取自**工具原始碼實際會印的字串**，工具改輸出而文件沒跟、或文件那節被刪，兩個方向都紅。

⚠ **這條紅線不必造變異——現況本身就是紅線樣本**：測試寫下去當場紅（Workflow 那條），
而同一支的另一條探針（「官方有、本機沒有」）是綠的 ⇒ 證明它分得出來、不是全紅。
修完 38／0、平台測試 34／0、回歸網 **1133/1133**。

⚠ **它綁不到的那一半要講明**：驗的是「**規格有沒有涵蓋工具印得出來的東西**」，
**不是**「模型回報時有沒有照做」——回報是模型產的字、不是程式輸出，測試看不到。
讀成「回報品質有守門」就又造出一個假承載體。界線同時寫在 `SKILL.md` §3.5 與測試 docstring。

**⑧ user 看了 02:28 的報告後直接退回，帶出三個問題**（2026-08-24 深夜）。

原話：「我還是看不出我自己有什麼 Skill 說明、用途」「官方的對比那些可以直接用官方的（如果有）」
「再來才是官方有我沒有的（建議加入流程）」「（Workflow 那段散文）這整段都是廢話阿」。

**問題一：⑦ 補的 §3.5 自己有 bug。** 我寫「官方說明（工具抓的·直接抄）」，
但 `officialCrossCheck.missingLocally` **只存名稱**（`skill_watch_run.py:529` 印的就是純名單），
說明其實在 `skills[]` 裡。⇒ **新室照我的指路做，那一欄整欄空白**，而它還誠實註記
「工具未抓到說明」——**是我指錯路，不是它偷懶**。

**問題二：「官方有、本機沒有」有假陽性。** 它量的是「**不在注入清單**」，而注入清單**只裝 skill**。
`claude --help` 的 `Commands:` 區列著 **`doctor`**，而 §18.7 ④ 那次 `/doctor` 是**真的跑成功**的
⇒ **內建 CLI 指令必然被誤報成「本機沒有」**。`debug` 疑似同類（有 `-d, --debug` 旗標）。
`verify`／`batch`／`run-skill-generator`／`design-sync` 三來源交叉後**最可能是真的沒有**，
但**未確證**——決定性的測法是**在互動 session 打 `/` 看自動補完有沒有它們**，5 秒可解。

**問題三：§18.3 那列「`research` 改混合：查用官方 `/deep-research`」是不可執行的。**
`deep-research` 是 `[Workflow]`，**不在基準 16 支裡** ⇒ 這台叫不到它。那句要訂正。

**修法**：`SKILL.md` §3.5 整節改寫成 **user 指定的三段順序**——
①我有什麼（自建分全域／專案，`Skill｜用途`）②我的哪些可直接改用官方的（四欄，
換不掉就明講「一支都不能換」＋共同理由）③官方有我沒有的→建議（三欄，`[Workflow]` 也放這段）。
同時修掉錯誤指路（說明去 `skills[]` 拿）、加上假陽性防線（宣告「本機沒有」前先查 `claude --help`）、
並明令**不要寫「值得你自己看一眼」那類散文**。

**測試加兩條探針，且刻意標明來源強度不同**：
(A) **原始碼推導**——`missingLocally` 存的是名稱列 ⇒ 規格必須指到別處拿說明（`skills[]`）；
(B) **規格要求，不是推導**——工具全程沒有任何可用性探測，假陽性只能靠規格擋 ⇒ 規格必須要求先查 `claude --help`。
**變異證明四種行為全對**：拿掉 `skills[]` 只紅 A、拿掉 `claude --help` 只紅 B、
工具若不再存 `missingLocally` 則 A **轉為「不適用」而非誤紅**。40 通過 0 失敗。

⚠ **把 (A)(B) 分開標是刻意的**：混在一起會讓「規格要求」借用「原始碼推導」的可信度。

**⑨ user 打了 `/` 之後，⑧ 的結論被放大成「那個標籤本身是錯的」**（2026-08-24 深夜）。

user 回報：**`verify` 與 `batch` 補完清單裡都有。** 加上已確認的 `doctor`
⇒ **6 支裡 3 支是假陽性**，而且 `verify`／`batch` **在互動 session 的模型技能清單裡同樣看不到**
⇒ **成因不是「無頭 vs 互動」**（W-8 那條），是**注入清單根本不裝內建 slash command**。

**⇒ 這支工具測不到「你到底有沒有」，它只知道「有沒有被注入給模型」。**
`officialCrossCheck` 這一半的語意從一開始就寫錯了。

**修法（三處一起改，否則措辭與判準會分家）**：
- `skill_watch_run.py`：印的字從「官方有、本機沒有的 Skill」改成
  「**官方文件有、不在注入清單**」，並當場多印一行但書；`newly` 那行同步改。
- `SKILL.md` §3.5：標題禁止寫「本機沒有」；每一支在講「建議加入」之前**先確認到底有沒有**
  （查 `claude --help` 的 `Commands:` 區，或請 user 打 `/` 看補完——5 秒最準）；
  查不出來的標「**未確證**」，不要因為它在表上就當成缺口。
- 測試補 (C) 回歸守門：工具若仍出現「本機沒有」字樣，**必須同時印出「不等於」的但書**。

**⚠ 這一輪最值錢的不是修好標籤，是當場踩到並修掉一個靜默退化。**
改工具措辭之後，斷言數從 **40 掉到 39 而沒有任何一條紅**——因為 ⑧ 寫的探針釘的是
**print 的字面值**（`"官方有、本機沒有的 Skill"`），字串一改就 `not in src` ⇒ **靜默跳過**。
⇒ 探針改釘**變數名**（`missingLocally`）：措辭可改、識別字不會；
跳過時**印出「探針不適用」而不是無聲 continue**；再加一條
「**探針沒有整批空轉**」（`ran >= 2`）——零目標不算通過。42 通過 0 失敗、回歸網 1133/1133。

**教訓可搬走**：**綁文件與程式的探針，錨要下在識別字（變數／函式名）上，不要下在會被改寫的顯示字串上。**
否則那條測試會在「有人改了措辭」的那一刻靜靜地停止工作，而畫面上全綠。

**⑩ 剩下四支也全部有 ⇒ `officialCrossCheck` 的「缺口」語意從頭到尾是錯的**（2026-08-24 深夜·定案）。

user 回報 `debug`／`deep-research`／`design-sync`／`run-skill-generator` **也全都補得出來**。
加上 ⑨ 的三支 ⇒ **6/6（含 Workflow 是 7/7）全是假陽性**。

**⇒ 這條訊號自建立以來一次真缺口都沒抓到過。** 它量的是「有沒有被注入給模型」，
而**注入清單只裝 skill**，內建 slash command 一支都不會進去。

**⚠ 我在 ⑧ 寫的訂正本身也是錯的**：那裡說「§18.3 的『`research` 改混合：查用官方
`/deep-research`』是不可執行的」——**`deep-research` 拿得到，那句話一直都是可執行的**。
我用一個壞掉的訊號去否定一份正確的紀錄。**⇒ 用某個判準推翻既有紀錄之前，先驗那個判準本身。**

**修法（第三處污染也一起清）**：除了 ⑨ 已改的兩處，`skill_watch_run.py:576` 寫進 `TODOS.md`
的摘要是「官方新增而**本機沒有** N 支——**可能需要升版才拿得到**」，**兩個斷言都是錯的**，
改成「官方文件新增 N 項——**注入清單沒有不等於拿不到**，打 `/` 看補完才算數」。
`SKILL.md` §3.5 的防線改成「**預設當成拿得到**（歷史命中率 0/6），要講缺口必須先證明」。

**留下來的真訊號只有一個**：`⚠ 官方文件新增 N 項`——「**官方出了新東西**」是真的，
「**你缺了東西**」不是。兩者在同一行講了三個月，現在拆開。

**這一整段（⑦→⑧→⑨→⑩）的形狀值得記**：四次都是同一個病的不同深度——
