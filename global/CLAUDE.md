...
- **改完名要推雲端**：`set_session_title` **只寫本機**；側邊欄／網頁／手機讀的是雲端那一份。改名後接著跑 `py -3 D:/Patrick-AI/.ai-harness/tools/push_cloud_title.py "同一個標題"`。**推失敗要當場講出來、不准吞**（它 exit 1 並印原因）。⚠ 憑證過期時它會叫你在終端機跑 `claude -p "hi"` —— **桌面版不刷新那份憑證檔**，所以這支每天會失效一次。
