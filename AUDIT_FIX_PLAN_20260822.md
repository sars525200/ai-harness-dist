...
- ✅ **群 B 的 B1／B2 已完成（2026-08-22）**；**B3 判定為假發現**；B4／B5 仍被佔用。
  - **B3 是假發現**（與看板「六大類」同一類：歷史敘述 vs 當下事實）。
    `HARNESS_PROGRESS.md` 的「2d 已做（187→115）」與「Phase 2：…白名單 187→115」
    都是**Phase 2d 做了什麼**的歷史紀錄，不是當下值的宣稱。改它會把歷史改成假的。
    ⚠ 附帶一個更強的理由：**allow 這個數字每天在動**——我上午量到 102（77＋0＋25），
    下午重量是 **115**（95＋0＋20），另一 session 在專案 `settings.json` 加了 18 條。
    「更新成正確數字」對這種欄位本來就是無效動作。
  - **B1／B2 是真的，而且比稽核列的多一處**：稽核列了 4 個位置，
    守門判準當場又抓到**第 5 個**（`## 5. Hook` 章節標題寫「6 條規則上線·3 條 enforce·3 條 shadow」，
    實際 12／12／0）。五處全部改成當場量到的值，並把「條數的單一真相是
    `dispatch_config.json`」寫進去、不再重抄清單。
    行 109 是**一行兩個錯**：角色數 4（實際 6）＋「檔案在 project 層」（2026-08-05 已搬到 harness，
    走 junction）。
  - **F-18 的守門做了**（覆核說「純手改會再漂且沒有守門會叫」）：
    `tests/test_hook_rules.py` 新增一個 case，斷言 `HARNESS_PROGRESS.md` 裡
    **同一行同時有 `enforce` 和「N 條」**的每一個 N 都等於 `dispatch_config` 的實際條數。
    掛在 `run_hook_tests` 裡＝**每次跑回歸都會被看到**（F2-9：問題不是沒有偵測器，
    是偵測器的 exit code 沒有人被迫看）。
    ⚠ 刻意**不新建測試檔**：註冊新檔要動 `tests/run_hook_tests.py`，而它的改動位置
    與另一 session 的 hunk 只隔幾行、會合併，所以掛進已註冊且乾淨的 `test_hook_rules.py`。
  - **紅燈三態**：①現況綠 ②把文件的 12 改回 9 → **紅並點名行號** ③把所有含 `enforce` 的行拿掉
...
- ✅ **群 C 已完成（2026-08-22）——而它從頭到尾就沒有被卡住。**
  - **原本的判定是錯的**：我把「`harness-dashboard.html` 有 4600 行 diff」讀成「另一個 session 在手改它」。
    ...
    `globalSkills: 2→8`、兩個分頁徽章）**也全是產生器寫的**。⇒ **零個手寫編輯**，
    它只是來源變了之後的重生漂移，而其中一部分是**我自己這幾個 commit 觸發的**
    （`capability_checks.py` 就在 `refresh_dashboard.py` 的 SOURCES 裡）。
  - **中途一個假結論被變異測試拆穿**：`refresh_dashboard --quiet` 跑完雜湊不變，
    我一度判「可重現」。插一個 `<!-- MUTATION-PROBE -->` 進 marker 區再跑 → **探針沒被洗掉**
    ⇒ 它根本沒重跑（來源雜湊快取）。**「雜湊不變」有兩種解釋，而我先取了想要的那一種。**
    改用 `--force` 才真的重生（探針被清、雜湊變動）。
  - **順帶抓到一條規則的缺口**：連跑兩次 `--force` **不冪等**，而
    `dashboard-generators.md` 只把成本／mix 列為「驗不了冪等」的資料源。
    實測差異落在 **`ROLES_TOPOLOGY`（2 處）＋時間戳徽章（1 處）**——
    前者從 event log 反推角色實派次數，**我每跑一個工具它就在長**，是同一類但規則沒寫到。
    ⇒ 登記進 `TODOS.md`。
  - **做了什麼**：①三處手寫矛盾改成當場量到的值（`10 條全數 enforce`→12、
    `9 條全數 enforce` 的規則清單重寫、`5 條規則已完工，皆 shadow`→`12 條，皆 enforce`）
    ②**整區刪掉「六大類總覽」手寫六卡**（user 定案）——它是正上方產生器區的
    **不完整過期複本**（只有 6/8 類，數字停在 7 月底），連同它的 subtab 一起移除
    ③**多修一處計畫書沒列到的**：`permissions` 那行的 `allow 115／deny 12`
    （F2-4 只抓到六卡區那個），改成當場量的 `allow 102／deny 27（Bash 13／PowerShell 14）`。
  - **`COST_PANEL` 區還原成 HEAD 版**：`gen_cost_panel.py` 刻意不在 refresh 鏈裡
    （`refresh_dashboard.py:103` 明寫），所以那一區的變動是**有人手動跑了那支壞掉的產生器**。
    它的 8 個測試現在是紅的，我無法驗證輸出正確 ⇒ **不把未經驗證的產物收進版控**。
    這是「只 commit 自己的」用在**區塊層級**。
  - 驗證：手寫修改在 `--force` 重生後**全部保住**；`test_dashboard_structure` 全過（10 頁籤）；
    最終 diff ＝產生器區 884 hunk ＋手寫區 10 hunk ＋ `COST_PANEL` **0 hunk**；回歸 938/947。
