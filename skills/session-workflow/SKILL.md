...
> **每次轉階段前，先往任務檔的進度日誌追加一段**（格式見 `/chat-handoff` §1）。
> 這不是額外工作 —— 轉階段本來就要重宣告，那一刻手上剛好有「做了什麼／為什麼／沒做的」。
> 事後回憶寫出來的交接檔必然過時，2026-09-03 實測六則接手對話全部因此重新核對一輪。
...
**完成判準**：宣告已寫出；S／M 已派出查詢（或寫出「L 級、一次 grep」）；沒找到的欄非空或明寫「無」；**S／M 的任務檔已建**（`/chat-handoff` §0，只寫目標與硬限制）。
...
- **沒點頭＝還在這一步。** 禁止邊做邊問——邊做邊問會讓人在**沒有全貌時做決定**。

**完成判準**：L 已聲明省 Design、S 已聲明不落檔、M 已提醒人打 `/wayfinder`；其餘每個分岔都有人的決定，且驗證方式已寫出「會紅的條件」；**進 Design 前已追寫 `[Research]` 那一段**。

### 3. 施作（照規格，不擴大）

畫面 → `visual-designer`（Cursor／Claude 都有）。規格已鎖定只差改檔 → `executor`。前端雙目錄另派 `sync-checker`。主則只留判斷。

**撞到範圍外但值得做的問題，二選一（2026-09-06 定）**：先判斷能否**獨立成一個任務**——
能（脫離這則對話脈絡也做得下去）→ 呼叫 `spawn_task` 產生建議任務卡；
不能（單純分心小事、離不開這則脈絡）→ 沿用全域 `CLAUDE.md` §3 Execute 的「記票不修」（寫進 issues 檔／`TODOS.md`）。
**不重複登記同一件事**——選了 `spawn_task` 就不再寫進 issues 檔。

**完成判準**：規格每一項都有對應改動或「沒做＋理由」；**進 Execute 前已追寫 `[Design]` 那一段**（含被否決的作法與理由 —— 那正是接手方最容易重跑一次的東西）。
...
**完成判準**：逐項判定有證據；回退時宣告已改回 Design 或 Execute；**進 Review 前已追寫 `[Execute]` 那一段**。回退時也追寫一段，寫明回哪一步與為什麼。

### 5. 交付

Review 清單每條 fixed 或 skipped（附理由）。發現要收斂到「**已修且已生效**」，不是「已知道」；部署過的要有 served 版本的實測輸出。人明確說才能推正式（`/deploy-prod`）。收工才 `/shougong`。

**完成判準**：沒有未標的發現；部署若發生則有 served 版本實測，否則寫進 `PENDING_VERIFY.md`；**任務檔已追寫 `[Review]`／`[Fix]`，並照 `/chat-handoff` §3 結案或留 `status: open` 與未完成清單**。
