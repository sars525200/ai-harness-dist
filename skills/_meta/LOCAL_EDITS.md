...
| `grilling` | 真 upstream | 1 | 2 | 1 | 1 | 0 | 0 |
| `prototype` | 匯入 `099f782` | 1 | 2 | 3 | 3 | n/a | n/a |
| `research` | 匯入 `099f782` | 1 | 3 | 3 | 3 | n/a | n/a |
| `to-tickets` | 匯入 `099f782` | 1 | 1 | 1 | 1 | n/a | n/a |
...
- `SKILL.md`：新檔行 3, 30–33；marker 在 33
- marker 對帳：逐檔通過（有分歧的檔都有 marker，有 marker 的檔都有分歧）。
...
- `SKILL.md`：新檔行 3, 29–31；marker 在 29
...
- `SKILL.md`：新檔行 3, 14, 16–36；marker 在 14, 19, 31
- marker 對帳：**不適用**。基準不是真 upstream ⇒ 早於基準的改動在 diff 裡看不見，那個檔會顯示「有 marker、零分歧」而被誤判成過期標記。**這是「量不到」，不是「對過了沒問題」。**
