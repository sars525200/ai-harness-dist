...
## 視覺爭議：先量再改（2026-08-26 從專案常駐層併入）

使用者說「歪了／沒對齊／看起來怪」時，**先量，不要直接改 CSS**。量完分三種，處置完全不同：

| 量到什麼 | 是什麼 | 怎麼處置 |
|---|---|---|
| 偏移 > 0 | **版面問題** | 照量到的值改盒模型 |
| 偏移 = 0，但**換縮放比例就歪** | **繪製期問題**（裝置像素對齊） | 改 margin／padding 沒用。**同一個元件禁止 CSS 盒與 SVG 各畫一半** —— 兩者的次像素捨入規則不同，湊在一起必然在某些縮放下對不齊 |
| 偏移 = 0，且**不隨縮放變** | **視錯覺** | 不要改。改了會讓量得到的那一半變壞 |

⚠ **`getBoundingClientRect()` 只量得到版面座標，量不到繪製期的裝置像素對齊。**
所以「rect 全是 0 偏移」不等於「畫面沒歪」—— 必須再換一個縮放比例截一次才能分辨後兩種。
