...
**檔頭一定要有 frontmatter**（2026-09-03 加）：
...
事情做完之後把 `open` 改成 `done`。**這一欄不是裝飾**：2026-09-03 實測 22 份
交接檔只有 4 份帶任何結案標記，其中 2 份是把狀態塞進檔名（`-done`、`-verified`）
—— 人本來就在標，只是沒有欄位可標。HND-1 靠這一欄判斷哪些還開著，
`tools/archive_handoff.py` 靠它決定搬哪些進歸檔區。**留空＝永遠開著。**
...
- **檢查它的指路還成不成立**：搬過家的函式、已作廢的「新對話建議第一句」。
  原文不要刪 —— 只加狀態、劃掉作廢的段落，當初的判斷過程還查得到。

搬進歸檔區用 `py -3 tools/archive_handoff.py`（不加旗標只印計畫，`--move` 才真搬；
只搬已標結案的，不刪檔）。

**完成判準**：磁碟上有那份 md、帶著 `status` 欄；人不必翻這則歷史就能在新對話開工。
