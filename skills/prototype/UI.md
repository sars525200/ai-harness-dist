...
Once a variant has won, capture the answer (which variant and why), then capture the prototype the way the [SKILL](SKILL.md) describes. Fold the winner into the real code and set the rest aside:

- **Sub-shape A**: fold the winner into the existing page; drop the losing variants and the switcher from main.
- **Sub-shape B**: promote the winning variant to a real route; drop the throwaway route and the switcher from main.

The full set of variants is the primary source, so it is kept, not binned — variant components and the switcher left wired into the live page rot fast and confuse the next reader, so unwire them but keep the files.

**LOCAL EDIT (2026-08-21): upstream moved the losers onto a throwaway git branch. No branch, no commit here** — list the files and hand the keep/branch/delete decision to the main session. Reason and reference: `SKILL.md` step 6, `SKILL_IMPORT_WAYFINDER_PLAN.md` K9.
