...
> **已退役（2026-09-03）——結論已併入 `TRIGGER_MECHANISMS_REFERENCE.md` §4，那裡才是現況。**
>
> 這份留著的理由只有一個：**推導過程**。「為什麼 R1 排第一」「為什麼 I1/I2 被降級」
> 的證據鏈在下面，併過去的只有結論。要重新審剩下那 50 條規則時，回來看這裡的方法。
>
> ⚠ **下面所有 `§8:102` 這種位置標記全部失效**——2026-09-03 追查確認，
> 4 條規則都還在，但都搬進 `.claude/skills/` 或 `.claude/rules/` 並改寫過，
> 對照表在 `TRIGGER_MECHANISMS_REFERENCE.md` §5.1。
> **索引記行號必然過期，這份就是活教材。**
