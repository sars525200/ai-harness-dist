...
| 5 | **Hook**（掛鉤） | 🟢 **18 條全數 enforce（0 shadow）** | 條數與 shadow 狀態的單一真相是 `hooks/dispatch_config.json`，**這裡不重抄清單**（重抄過一次，規則從 9 加到 12 之後這一格掛了兩週沒人發現）。交付形態分三種：BLOCK→exit 2、WARN→`additionalContext`、Stop 落便箋→UserPromptSubmit 投遞。便箋分**狀態型／事件型**（8/28）：狀態型不過期、超量時最後才丟，且規則的「同一則只講一次」以**投遞成功**為準（回執在 `contract.py`）|
...
## 5. Hook 🟢 → **19 條規則已註冊**（單一真相是 REGISTRY／`hooks/dispatch.py`）
　　　　`hooks/dispatch_config.json` 判定其中 **18 條 enforce**；IDX-1 另外一條刻意留在 shadow（8/27 上線，判準見 TODOS.md）
...
> ⚠ **實際 18 條，下表只列 6 條** —— `ENC-1`／`BUDGET-1`／`DECL-1`／`CTX-1`／`WIN-1`／`EXP-1` 是這張表寫成之後才加的，
