...
## 5. Hook 🟢 → **`dispatch_config.json` 登記 29 條**（28 enforce、1 shadow：DECL-2 —— 2026-09-07 補進設定檔明寫 shadow，在那之前它與 IDX-1 靠 dispatch.py 的預設值跑 shadow，IDX-1 就這樣判定 242 次、真的送達 1 次；同日 IDX-1 補回歸網＋變異後轉 enforce，並改成只在有可疑項或判斷不出來時出聲）——單一真相是 REGISTRY／`hooks/dispatch.py`，這裡的數字會漂，出問題以 F-18 守門的即時計數為準；2026-09-06 補 LEARN-1 時順手核對，發現這裡原寫 23 但設定檔實際已是 24——漂移早於本次改動
