...
## 5. Hook 🟢 → **`dispatch_config.json` 登記 24 條**（23 enforce、1 shadow：LEARN-1；IDX-1 額外靠 dispatch.py 預設值跑 shadow，不在這份設定檔裡）——單一真相是 REGISTRY／`hooks/dispatch.py`，這裡的數字會漂，出問題以 F-18 守門的即時計數為準；2026-09-06 補 LEARN-1 時順手核對，發現這裡原寫 23 但設定檔實際已是 24——漂移早於本次改動
