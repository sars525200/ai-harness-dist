...
> 本檔保留為「為什麼不做」的紀錄＋裁定衍生的四張票（T3／T4 已完成，T6／T7 記票）。
...
**Q4 · 備份三層現在到底在保什麼？**【**記票不修·要 user 決定**】
...
**分支指令**：`py -3 tools/check_before_start.py`（那一段就是要決定留不留的東西）。
...
| T6 | Q4：離線 `.bundle` 那一層留不留 | ⏸ 記票·等 user 決定 |
