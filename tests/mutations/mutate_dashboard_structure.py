...
MUTATIONS = [
    ...
    (
        ...
        # 2026-09-07 再換一次：原本綁 `"builtin": false`，那串在產物裡有 **6 份**
        # 逐字同文（每個自建角色一份），`replace(…, 1)` 只換第一份。逐份實跑六份
        # 都紅，但「第一份是誰」由產生器排序決定、沒有人在維護；而 `"builtin"`
        # 前面隔著 state／stateCls 這些會隨使用次數變動的欄位，湊不出又唯一又穩定
        # 的錨點。改綁識別字本身：它唯一、來自檔名所以穩定，撞的是同一條判定
        # （看板列出的自建角色 == agents/*.md）。
        "角色識別字漂掉（＝看板列出的自建角色與 agents 目錄對不上）",
        '"agentType": "executor"',
        '"agentType": "executor-DRIFT"',
    ),
    (
