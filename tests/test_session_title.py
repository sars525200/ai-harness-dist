...
case("任務帶短 id", "TITLE-1 格式：【分類·短id】名稱｜階段｜進度%",
     M.compose("任務", "三段式命名", "Execute", "60", "1a2b3c4d"),
     "【任務·1a2b3c4d】三段式命名｜Execute｜60%")
case("短 id 只取前 8 碼", "完整 session id 是 uuid，塞進去會吃掉 48 字上限",
     M.compose("任務", "三段式命名", "", "", "1a2b3c4d-0000-1111-2222-333344445555"),
     "【任務·1a2b3c4d】三段式命名")
case("討論也帶短 id", "分類三種都走同一個標記位置，不是只有任務",
     M.compose("討論", "命名方式討論", "Research", "20", "1a2b3c4d"),
     "【討論·1a2b3c4d】命名方式討論")
case("收尾也帶短 id", "收尾那一輪同樣要指得出是哪一則",
     M.compose("收尾", "收工封存", "Execute", "90", "1a2b3c4d"),
     "【收尾·1a2b3c4d】收工封存｜收尾")
...
case("宣告抽取一路帶著短 id", "compose 有了參數也要真的接上去，否則閘門那邊拿到的還是舊的",
     M._declared_task([_DECL], "", "1a2b3c4d"), "【任務·1a2b3c4d】修進出庫同步｜Execute")
...
# compose_closed：對話**已經結束**時的名字（user 2026-09-03 選的格式）。
# 與 compose_idle 分家的理由見函式 docstring —— 新視窗沒有名字，已結束的對話有。
case("結束時保留真任務名", "真名被蓋成『等待任務』＝回頭找對話時沒有線索",
     M.compose_closed("【收尾】SG-093 流程說明稿｜交接"), "【閒置】SG-093 流程說明稿")
case("階段與進度不留在閒置名裡", "那兩欄講的是進行中的狀態，對已結束的對話是雜訊",
     M.compose_closed("【任務】改權限判定｜Execute｜40%"), "【閒置】改權限判定")
case("猜來的【待】不升格", "那是猜的不是宣告來的，當『這則做過什麼』會騙人",
     M.compose_closed("【待】改權限判定｜AI-Projects"), "")
case("平台快取名不升格", "client 記憶體裡的舊值不是任務名",
     M.compose_closed("UI / 排版設計 (S)"), "")
case("沒名字回空字串", "回空才會退回 compose_idle，回別的會讓呼叫端誤判成有值",
     M.compose_closed(""), "")
case("重跑不會降級", "封存重跑時讀到自己上次寫的閒置名，不可以再被剝一層變成空",
     M.compose_closed("【閒置】SG-093 流程說明稿"), "【閒置】SG-093 流程說明稿")
case("閒置名算我們寫的", "認不出來就會被當成 client 快取名，log 的來歷標記會反過來",
     M.is_ours("【閒置】SG-093 流程說明稿"), True)
