...
| 項目 | 現況／為何還沒做 | 下一步（逐字指令或動作） | 誰 | 分類 | 優先 |
|---|---|---|---|---|---|
| 🔧 **唯讀閘門：缺專案側的 `py_compile`** | `sync-checker` 的工作是「兩端各跑一次語法檢查」。JS 六支 `node --check` 全過，**Python 六支全部跑不了** —— 閘門只放行 `D:\.ai-harness` 底下的 `.py`，且明確擋 `-m`。而本專案雙改清單實際含 `server.py` 與 `db/*.py` ⇒ **雙改檢核在 Python 側是永久盲區**。繞法是寫進報告請主 session 代跑（2026-08-18 我確實代跑了六支、全過） | `agent_readonly_gate._decide_py` 放行 `py -3 -m py_compile <檔>` / `python -m py_compile <檔>`：只讀檔產 pyc，**與既有的 `node --check` 同風險等級**。⚠ `-m` 目前是被整個擋掉的，要開的是這一個 module 不是所有 `-m`。驗收：唯讀角色能對 `SOP_PROD\05_UI_Demo\server.py` 跑出語法判定 | 待判斷 | 閘門 | 中 |
| 🔧 **唯讀閘門：只能語法檢查、不能跑測試** | `project-auditor` 要驗「測試兩端全綠」，但白名單只有 `node --check`、**沒有 `node <script>`** ⇒ 它只證得了「斷言文字改對了」，**證不了它是綠的**。這是稽核最常見的退化形狀：把「讀起來對」當成「跑起來對」 | 放行 `node <限定樣式的測試檔>`（例如只認 `test_*.js`／`*_test.js`，不放行任意腳本）。⚠ 風險比 `py_compile` 高一階——測試腳本會執行任意程式碼。要嘛限定目錄＋檔名樣式，要嘛不做並**把「稽核驗不到測試綠燈」寫進角色描述**，別讓人以為稽核涵蓋了那一項 | 待判斷 | 閘門 | 中 |
| 🔧 **唯讀閘門：`ls` 被擋，而盤存目錄是稽核的第一個動作** | 2026-08-22 票 08 稽核開場要盤存目錄，`ls -la <dir>` 與 `ls -R <dir>` **兩條全被擋**（前者還因 `\|\|`／`;` 命中元字元規則），**整個第一輪 tool block 零產出**，改用 4 次 `Glob` 才補回同樣的清單。⚠ **與其他項不同：這一項每次稽核都必然發生**，不是偶爾才需要的能力 | 放行 `ls` 的唯讀形式：`ls`／`ls -la`／`ls -R`，**限單一路徑引數、不接管線與 `;`／`&&`**。零副作用、與現有白名單同風險等級 | 待判斷 | 閘門 | 高 |
| 🔧 **唯讀閘門：查不到「檔案是不是連結」** | `harness-auditor` 要查 `~\.claude\skills` 此刻是不是 junction、指向哪裡。`ls -la` 被擋，而 **`Read`／`Glob` 會穿透連結、完全看不出儲存形態** ⇒ 它只能退回引用 CPython 文件行為＋別人留下的變異紀錄，「junction 斷掉會不會被發現」這一題**只答得出「程式碼路徑上不會」，答不出「此刻有沒有正在發生」**（主 session 代跑 `os.path.samefile` 三秒就有答案：兩條都健康） | 放行一支唯讀的儲存形態查詢：`os.path.realpath`／`os.lstat().st_reparse_tag`／`dir /AL` 任一。⚠ 若上面那條 `ls -la` 開了，這一項自動解決 —— **先做 `ls` 那條，再看這條還成不成立** | 待判斷 | 閘門 | 低 |
| 🔧 **唯讀閘門：稽核不到 VM（風險最高的一項）** | `project-auditor` 要稽核「部署→遷移→dump 重生」整條鏈，但 `ssh`／`curl -I`／`sqlite3 SELECT` 全不在白名單 ⇒ **只能抄被稽核者的自陳**，稽核變成「稽核者相信被稽核者」 | 二選一，**別留在中間**：①放行嚴格限形的唯讀遠端探測（`curl -sI`、`ssh <host> "sqlite3 file:...?mode=ro ... SELECT ..."`，**只許 `mode=ro` 與 `SELECT`**）②不做，但**把「稽核不涵蓋 VM」寫進角色描述**。⚠ 現況是最糟的第三種：沒能力又沒寫明，讀報告的人會以為涵蓋了 | 待判斷 | 閘門 | 中 |
