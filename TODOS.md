...
>   分類沒填則整個 chip 不出現（空 chip 比沒有 chip 更吵）。
>
...
| **Skill Eval L4：15 支 skill 尚未實跑驗收**（**8/20 訂正：不是 8 支**） | L1/L2 全自動已綠，但那不含「跑起來對不對」。8/20 實查 `eval\acceptance.json` **全檔只有 2 筆**（`license-rules` 與 `shougong`，都停在 2026-07-28），而 `SKILL_EVAL_PLAN.md:512` 已把總數訂正為 **17 支** ⇒ 未驗收是 **15 支**，且**二十多天沒有新增任何一筆** | 逐支照 `/verify-skill` 的三層做一次真實 dry-run；spawn-agent 型的每個分支都要跑到。⚠ 先看 `acceptance.json` 為什麼沒人在寫——**二十天零新增**比「還有 15 支要做」更值得先查：那可能代表寫入那一步根本沒有人在執行 | 逐支補 | 流程 | 高 |
...
| 🔧 **eval L2 對「skill 同目錄檔案」解析不到 —— 假紅**（2026-08-23 閘門清理線發現） | `eval\check_contracts.py` 的 `SEARCH_BASES` 沒有「skill 自己的目錄」，且只寫檔名時的遞迴**限一層深度** ⇒ 構不到兩層深的 `skills\skill-watch\run.py`。**檔案真的在**（`git ls-files skills/skill-watch/` 列得出來），所以是假紅不是缺檔。今天才浮出來的原因：HEAD 版只掃專案層一層、看不到 harness 層的 skill，是**案 A 的 A-3「掃兩層」（工作區未 commit）**第一次把它納入檢查；而 8/16 施作 A-3 時 harness 層只有 `context-health`／`visual-check` 兩支、都沒有同目錄檔案引用，所以當時驗不出來。⚠ `SKILL_EVAL_PLAN.md` §9.6b「案 A 範圍外、已知但不做的」六條**沒有涵蓋這一條** | 完整根因（定位到行）·修法·四步驗證（含「先證明它會紅」的反向項：把 `run.py` 暫時改名必須重新變紅）·沒找到的 → `.scratch\room-gate-cleanup\FINDINGS.md` 發現 1。⚠ **不要放進 `contract_allowlist.json` 豁免**——檔案真的存在，豁免等於把檢查器的能力關掉。⚠ 它在別條線正在重構的同一區（U-1 硬編碼還債），**該由那條線順手做**，別兩邊各改一次 | 待判斷·歸 eval 線 | 流程 | 中 |
| 🔧 **案 A（eval 子系統修復）完成並驗過一週，整批從未 commit**（2026-08-23 實查） | `SKILL_EVAL_PLAN.md` §9.6b：A-1～A-9 全打勾、`VA-1～VA-9 全過`（每項都寫了「紅線怎麼證的」），案 B 未動工 ⇒ 這是一個乾淨的完成點。但實查 `git status`：`config.py`（A-1 新增的共用 loader）至今 **`??` 未追蹤**，`eval\check_structure.py`／`run_all.py`／`check_contracts.py`／`check_acceptance.py`／`run_triggers.py`／`baseline.json` 與 `tests\test_harness_config.py` 全是未 commit 的 `M`，最舊 mtime **2026-08-16**。這個 repo 有外部程序定期 `git add -A`、各線收工也可能 auto-commit ⇒ 一週的已驗成果躺在髒工作區，被別人的 commit 掃進去之後**分不出是誰的。**2026-08-23 晚更新：`config.py` 已被看板線 commit**（`refactor(dashboard): U-1` 那筆）——**不是代為結案**，是 U-1 把 5 支 dashboard 檔改成 import 它，不一起 commit 的話 HEAD 上的看板程式會 ImportError。**其餘 `eval\*` 六個檔與 `tests\test_harness_config.py` 仍全數未 commit，那顆按鈕還在你手上**** | 先問當事人一個問題：**「刻意不 commit」還是「忘了」**——`TODOS.md` grep `U-1`／`config.py` 零命中、§9.6b 也沒有暫緩字樣，這兩者從檔案上分不出來。健康度已實跑不必重量：`py -3 tests\run_hook_tests.py` **1033/1033**、`py -3 eval\run_all.py` 只有 L2 那個假紅（見上一列）。第二個分岔：`check_contracts.py` 的 8/22 改動比其餘檔晚六天，要不要一起 commit **未查證**。細節 → `.scratch\room-gate-cleanup\FINDINGS.md` 發現 2 | 待判斷·功勞歸屬不代按 | 備份 | 高 |
...
| 🔧 **`tools\peek_sessions.py` 在 cp950 終端崩掉，看不到別的 session 在做什麼**（2026-08-23 覆核線踩到） | 它印 session 摘要時帶 👤／🔧／💬 emoji，Windows 預設 cp950 編不出來 ⇒ `UnicodeEncodeError: 'cp950' codec can't encode character '\U0001f464'` **在印到第二個 session 就整支中斷**。危險的地方是它**不是空輸出而是半截輸出**：第一個 session（剛好是我自己）印完了才炸，看起來像「只有我一個在跑」。這次的實際後果是我差點據此判斷「沒有別的 session 佔用看板 HTML」；繞法＝`PYTHONIOENCODING=utf-8` 前綴，重跑才看到**同時有 4 個 session 活著**。同源：`gen_layers` 的 `SystemExit` 也走非 `_force_utf8_output()` 的路徑 | 給 `peek_sessions.py` 補 `_force_utf8_output()`（hooks 側已有現成實作），或至少把 print 包 try/except 讓它印得出 ASCII 退化版。**驗法**：不帶 `PYTHONIOENCODING` 直接 `py -3 D:\.ai-harness\tools\peek_sessions.py`，應完整印完所有 session 而非中途 traceback | 待判斷 | 流程 | 中 |
| 🔧 **升版 Claude Code 並評估 `/doctor` 是否取代自建的 `context-health`**（2026-08-23 `/skill-watch` 判定·user 定案「升版，然後評估」） | 這台是 **2.1.143**。官方 `/doctor` 的職掌與 `context-health` 高度重疊——它明文寫「trims checked-in `CLAUDE.md` files by cutting content Claude could derive from the codebase, and migrates the always-loaded guidance that remains into skills」，那就是 `context-health` 在做的事，而且它**多做**「找出沒在用的 skill／MCP／plugin 對照它們的 context 成本」。⚠ 但那個 CLAUDE.md 瘦身功能要 **v2.1.206+**，`/dataviz` 要 **v2.1.198+** ⇒ 現在比不了。連帶卡著的還有 `/run`／`/verify`／`/batch`／`/debug`／`/run-skill-generator`／`/design-sync` 六支本機沒有的 | ①升版（user 自己跑，不要在 session 中途由我升——會動到正在跑的這支 CLI）②升完跑一次 `/doctor`（它會先報告再問要不要改，不會自己動手）③拿它的實際輸出與 `context-health` 逐項比 ④判定：剃除 `context-health`／保留兩支各司其職／把 `context-health` 縮成 `/doctor` 沒做的那部分。⚠ 升版後也**重跑一次 `/skill-watch`**，那六支會從「本機沒有」變成新增，是預期的不是異常 | user 升版·我評估 |
| 🔧 **跑一次 `/fewer-permission-prompts` 產唯讀指令 allowlist**（2026-08-23 `/skill-watch` 判定·user 決定開始用） | 官方內建這支：掃 transcript 找常用的唯讀 Bash 與 MCP 呼叫，產出優先排序的 allowlist 寫進專案 `.claude/settings.json`。記憶檔 `feedback-atomic-commands-permission-prompts` 整條就是在講「權限詢問一直跳」，而**今天這一輪就被擋了一次**（`git checkout --`）。⚠ 沒跑是因為它會改 `settings.json` 的 permissions，那是使用者可見設定、且多 session 共用——不該由我順手改 | 打 `/fewer-permission-prompts`，看它提的清單再決定收哪幾條。⚠ **收之前先確認它提的每一條真的是唯讀**——`git checkout` 這種名字看起來像查詢實際會毀資料的不可放行 | user |
| 🔧 **Workflow 類別沒有基準 ⇒ 官方新增一個 workflow 不會被報成變動**（2026-08-23 修一半） | 交叉驗證原本只比 `[Skill]` 那一類，而 `/deep-research` 被官方標成 `[Workflow]` ⇒ **它從來沒被端到人面前過**，儘管 `skill-watch` 的開場白自己就在講「我們正打算自己包一支功能更弱的」講的就是它。**已修一半**：現在會把 workflow 逐項印出來附說明（`skill_watch_run.py` 的 `[3/6]` 那段）。**沒修的一半**：workflow 不進 `baselines`，所以「官方**新增**了一個 workflow」這件事偵測不到——而那正是這支工具存在的理由 | 決定 workflow 要不要進基準。⚠ 它與 skill 的形狀不同（不進注入清單、比不出「本機有沒有」），所以不能直接套 `baselines[平台][模式]` 那個結構——可能要另開一個 `officialWorkflows` 快照，比對邏輯也另寫。做之前先想清楚「workflow 消失了」要不要報 | 待判斷 |
| 🔧 **平台 skill 變動偵測：新增 7 支（design, dataviz, artifact-design, artifact-diagramming, artifact-capabilities, code-review, run）；消失 1 支（review）** | 排程於 2026-08-24T00:09+0800 偵測（headless／中性目錄）。基準擷取於 2026-08-22T23:39+0800。改名候選：[('review', 'code-review', 0.706)]。 | 逐支判斷是否與現有全域技能重疊（可合併／可取代）。機制與判準見 D:\.ai-harness\SKILL_WATCH_PLAN.md（§4 分岔決定、§11 覆核紀錄）。判定完把這一列刪掉；要保留判斷結果就搬進該計畫書。 | 待判斷 |
