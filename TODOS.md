...
|---|---|---|---|---|---|
| **看板 Skill 清冊徽章 28 vs 實際 18**（2026-08-25） | `test_dashboard_structure` 結構驗證失敗 → 8099 watcher 不寫雜湊、曾每 10 秒 `force=True` 全量重生（backoff 已補）。徽章計數與 `skills/` 目錄不一致 | 對齊產生器計數與 `skills/` 實數（18），或改徽章資料源；改完 `py -3 tests/test_dashboard_structure.py` 應過，8099 log 應回到 10 秒成功節奏 | 下次施工 | 看板 | 高 |
| **填滿的 `harness-dashboard.html` 不該進 git**（2026-08-24 立案·§8 已決） | 殼＋產物＋測試 golden 焊在同一檔；8099 每 ~10 秒重生 → 工作區常髒。切法與已決項在 `DASHBOARD_HTML_GIT_PLAN.md`：徽章只注入、啟動自動 generate、CI 只測 fixture。程式尚未動 | user 指示開工後：照計畫拆 `harness-dashboard.shell.html`、gitignore 填滿 html、測試改 fixture、8099 啟動時從 shell 複製並填 | 等開工指示 | 看板 | 中 |
