...
| 項目 | 現況／為何還沒做 | 下一步（逐字指令或動作） | 誰 | 分類 | 優先 |
|---|---|---|---|---|---|
| 🔧 **唯讀閘門：`ssh` 遠端唯讀查詢（`curl -sI` 已放行）** | 2026-08-23 已放行 `curl -sI`（HEAD 探測），所以「站台活著沒、端點回什麼狀態碼」驗得到了。**剩 DB 與檔案落點**：`ssh`／`sqlite3` 仍擋 ⇒ 「部署→遷移→dump 重生」只驗得到最外層 HTTP。⚠ **這一條的風險等級與前四條完全不同**：`CLAUDE.md` §9 記著 VM 的 SSH 是**金鑰＋NOPASSWD sudo**，而 `ssh <VM-HOST> "sudo rm -rf /srv"` **不含任何 shell 元字元**——現有的元字元守門攔不到它。白名單只要有一個縫，後果不是「看到太多」是「對正式機下 sudo」 | user 已表示要做（限 `mode=ro` 的 SELECT），但因上述風險**等再確認一次才動**。要做的話：遠端指令必須整串比對通過嚴格樣式（`sqlite3` 開頭、含 `mode=ro`、SQL 以 `SELECT` 起始、拒絕 `.` 開頭的 dot-command —— `sqlite3 db ".shell ls"` 會執行 OS 指令），並附對抗式測試（`sudo rm`／`DROP`／`mode=rw`／`.shell` 各一條必須被擋） | 待 user 再確認 |
