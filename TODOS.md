...
| 🆕 **UI-1 接了線但從未產出 findings**（8/20 由新的 findings/applies 指標當場抓到） | `report.py` 新增「findings ／ applies」對照欄之後，第一次跑就抓到：**UI-1 applies 185 次、findings 0 次**，而它**不在本表任何一列**。這是「分母有值、分子恆為 0」的**第四例**（前三例 R4／R1／DECL-1 都是判準綁錯層）。⚠ 同表的 R1（456／0）是已知死規則、R3（456／0）是已知時間差，UI-1 兩者都不是 | 照 R4／R1 的做法：找一個**已知該被抓到**的歷史樣本（UI-1 是 `ui1_variant_parity.py`，變體對稱性），看它會不會命中。不會就是判準綁錯層。⚠ 先確認它不是第二個 R3（判準對、只是情境還沒發生）——分辨方法是拿歷史 commit 回放 | 下次施工 |
