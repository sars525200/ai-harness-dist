...
`D:\Patrick-AI\.ai-harness\` 是 hooks 實作、dashboard、tests、reviewer 的實體所在，
...
| D2 | `D:\Patrick-AI\.ai-harness\` 歸哪層 | 併專案層／獨立第三層／標成「共用元件」 | **標成共用元件**：它不是 Claude 自動載入的層，但被專案層引用。獨立成第三層會讓切換鈕變三段、而它其實沒有「切過去看」的意義 |
