...
| Q3 | 驗收過期判定 | **bundle 內容雜湊變動 OR L2 契約檢查失敗** ⟶ 過期。**不用日曆天數**。<br>⚠ **2026-09-07 修正：原定案是「檔案 mtime 變動」，已改為內容雜湊。** mtime 量的是「檔案被寫過」不是「內容變了」⇒ `git checkout`／clone 到新機器／`touch`／行尾 CRLF↔LF 轉換都會讓**全部 33 支同時轉過期**，而那正是 Q3 想避免的「週期性假警報」換一個觸發源重演。雜湊前先正規化行尾與行尾空白。**仍未涵蓋**：改一個錯字照樣轉過期（機器分不出實質與否）。判定式收斂在 `check_acceptance.judge()` **一處**，看板從那裡借（原本自己抄了一份，換制時會靜默失效） | 沒改過的 skill 不會因時間流逝而失效；會失效的是它的依賴，那由 L2 抓。日曆天數會對穩定的 skill 產生週期性假警報，而假警報會讓人開始無視整套 eval（§5.5） |
...
| **A-6** | `check_acceptance.skill_hashes()` **一處**（2026-09-07 前叫 `skill_mtimes()`，見 Q3） | 新鮮度來源改吃 `SKILL.md + 全部 bundle 檔`。不改的話案 B 之後「改一條搬進 references 的硬規則不會動到 SKILL.md」⇒ L4 永遠顯示「有效」，§7 Q3 定案對搬走的內容整個失效（R2-6）。⚠ v4・R3-13 刪掉原本寫的「`check_structure` baseline」那一半——`baseline.json` 的 `mtime` **全檔沒有任何消費者**（`:200` 只讀 `tokens`），改它不會讓任何檢查會叫，寫成兩處會讓人以為有兩道防線 |
