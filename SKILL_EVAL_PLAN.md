...
1. **第 1～12 列全數 `verified`**（本地批 7 支＋外部批 5 支，其中 3 支零改動）；**只剩第 13 列 `wayfinder`**；目前沒有 active 列。
...
- **本地批（第 1～7 列）累積、不在單列允許 path 的帳**（2026-08-26 收齊，逐項可直接施工）：
  ...
  - `skills\design-spec\SKILL.md:78` 的絕對路徑 `` `D:\.ai-harness\STOP_HOOK_MARKER_PLAN.md` ``
...
- **第 12 列（`to-tickets`）帶出的三筆**（2026-08-26）：
  ...
  - **⚠ 專案端 `.scratch/` 不是 gitignored**（實測 `git check-ignore` exit 非 0，11 張票檔是已追蹤的），
    而 harness 端是（`.gitignore:11`）。配上「外部程序定期 `git add -A`」⇒ **任何在專案端 `.scratch/`
