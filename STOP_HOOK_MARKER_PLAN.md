...
| `_entry_probe` 臨時診斷碼 | ✅ **2026-08-20 已移除**，結論見 §5.1 |

### §5.1 `_entry_probe` 的最終量測（2026-08-20·移除前留檔）

2026-08-05 為了查「Stop 事件 5 個回合只有 3 次落進 events log」，在 `dispatch.py` 的
**任何解析與判定之前**加了一支無條件寫檔的 probe（`state/stop_probe.log`）。判讀模型是：

> probe 筆數 == events 筆數 → Claude Code 沒呼叫（問題在上游）　　probe > events → 呼叫了但 dispatch 沒走完

8/06 先定案原始疑慮**不是 bug、是分母算錯**。8/20 移除前用它累積的 15 天資料做最終量測，
時間窗對齊到 probe 起始的 `2026-08-05T22:36:37`（events 本身回溯到 7/28，不對齊會得到
662 vs 939 這種假落差）：

| 事件 | probe（解析前就寫） | events（走完才寫） | 差 |
|---|---|---|---|
| `Stop` | 481 | 480 | **+1** |
| `SubagentStop` | 181 | 181 | **0** |

**結論：662 次呼叫只有 1 次沒走完（0.2%），`dispatch` 實質上每次都跑完。**

移除的理由不只是「問題答完了」——它累積到 **21,436 行／958KB**（登記時 463 行／20KB，**46 倍**），
其中 **74% 是 `PreToolUse`**（15,875 行），而它是為了診斷 `Stop`（481 行、2.2%）才存在的；
全 repo 只有寫入端 `dispatch.py:560` 提到它，**沒有任何讀取端**；而且它在 hot path 上，
每一次工具呼叫都 open + append + close 一次。它自己的 docstring 就寫著「查清楚後移除」。

⚠ **它答不了的那一題**：同批登記的殘留②「壓縮結尾的回合會漏掉 AWC-1／BUDGET-1／PR-1」
**不在這支的量測範圍內**——probe 在解析之前就寫，量的是「dispatch 有沒有被呼叫」，
而②問的是「規則有沒有被評估」，是不同層。那條仍掛在 IT 專案的 `PENDING_VERIFY.md`，
要另做機制（例如在規則評估完成後補一筆 event，才量得到「呼叫了但沒評估」）。
