...
| ✅ 已驗 | 看板 Hook 表已長出 MAP-1 那列（2026-09-08 00:00） | 先前「Stop hook 會自動刷新」的假設是錯的：hook 規則表是**離線產生器**（`refresh_dashboard.py` 的 `OFFLINE_GENERATORS`，刻意不進熱路徑），只在收工流程或手動跑 `dashboard/gen_hook_rules.py` 時重生——本次手動跑一次，HTML 出現 MAP-1。另查到第二個原因：本機看板服務（8099）自 2026-09-07 17:42 後無聲停掉（無重開機、無 python 崩潰紀錄、hook 事件無 kill），已用 Startup 的 VBS 重新拉起、回 200；死因未查明，已開任務卡「看板服務死了要有人發現」。`check_freshness.py` 仍 exit 1（規則數差異要再跑 `gen_task_flow.py`、維運腳本數要跑 `gen_layers.py`、然後 `--write-snapshot`）——那是收工流程 3.5 的事，本次未做。 |
