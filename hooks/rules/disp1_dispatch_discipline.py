...
CLAUDE.md §4.1：唯讀搜尋／跨檔盤點／事實查證／歷史追查一律派出去，不用每次問，
主 session 只留判斷與統合（2026-09-01 起，「要不要派工」仍不用問，但派之前要先過一輪
§1「動工前先問為什麼」——這條閘門盯的是「派工類型判斷」沒被執行，跟那條無關）。
...
def check(ctx):
    ...
    return warn(
        ...
        "CLAUDE.md §4.1：唯讀搜尋、跨檔盤點、事實查證、歷史追查"
        "一律派出去不用問，彼此不相依的同時派，主 session 只留判斷與統合。"
