...
def _is_large_plan(path: str) -> bool:
    ...
    **刻意留在 `_tail_lines`（2026-09-07 覆核後保留現狀，不是漏改）**：
    這支問的是「最後一次」，反向掃、命中即停 ⇒ 檔尾就是正確範圍，
    跟 EXP-1／LEARN-1 那種「整則有沒有出現過」不是同一類問題。
    真的一路掃到窗底都沒看到規模宣告時回 False，方向是**多講一次提醒**，
    與本函式既有的 fail-open 方向一致。
    """
    if not path:
        return False
