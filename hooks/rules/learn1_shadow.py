"""LEARN-1 —— Stop：碰技術面任務卻沒問過「要不要學」時提醒一次。
...
## 2026-09-06 轉正式（原本刻意留在 shadow）

user 選擇直接補上送達（「兩個都做」），跳過了本檔原訂「攢幾則資料再議」
的門檻——**這是知情的取捨，不是漏做**：判準用階段欄當代理指標，非技術面
的 Execute／Design 一定會有假提醒，所以送達文字明講「不是技術任務就忽略」，
且一則對話只提醒一次。誤報率的實測仍留在 `TODOS.md` 的待驗票上；若實際
干擾過大，回頭收緊 `applies()`（例如加上「這輪真的有動檔」），**不要直接
關回 shadow**——關掉就回到原本零紀錄的狀態。

以下是原本留在 shadow 的理由，保留備查：

## 為什麼原本刻意留在 shadow（不進 dispatch_config.json）
...
def check(ctx):
    ...
    return warn(
        f"LEARN-1：這則對話已經進入「階段 {stage}」，但整段對話沒有問過"
        f"「要不要附學習說明」的選擇題，使用者也沒有說「照做就好」。"
        f"若這是技術面任務，現在補問一次（選項依規模給推薦：輕量推薦不需要、"
        f"標準／大型推薦要一頁圖解）。若這其實不是技術面任務，忽略這則提醒即可"
        f"——判準用的是自我宣告的階段欄，本來就會涵蓋非技術的施作／設計。"
        f"這則對話只會提醒這一次。"
    )
