...
r"""產生看板「hook 規則表」：applies／would-block 的即時計數。
    ...
    py -3 D:\Patrick-AI\.ai-harness\dashboard\gen_hook_rules.py           # 注入 HTML
    py -3 D:\Patrick-AI\.ai-harness\dashboard\gen_hook_rules.py --check   # 只印，不寫檔
...
DESC = {
    ...
    "HND-1": {
        ...
        "tip": "動機是實測：<b>22 份交接檔只有 4 份帶結案標記</b>，其中 2 份是當天才補的；"
               ...
               "——人本來就在標，只是沒有欄位可標。"
               "刻意<b>只 WARN、不自己搬檔</b>：查到的先例裡，stale bot 導入後第一年每月活躍"
               "貢獻者 <b>-14%</b>、某專案每月 <b>37.3%</b> 的 open PR 被 bot 關掉；"
               "WordPress 2021 面對 2,700+ 張單，吵完的定案是「只貼標籤請人確認，不自動關閉」。"
               "判準最弱的地方不接最不可逆的動作。"
               "精準度是這條的命門，開發當天走過三版：裸檔名也算 → <b>24 筆、大半誤報</b>；"
               "只認帶斜線的 → 剩 2 筆、<b>兩筆仍是誤報</b>（指的是別的 repo）；"
               "加上鄰居 repo 比對 → <b>0 筆誤報</b>。"
               "成本靠目錄簽章擋在前面（檔數＋最新 mtime，只做 stat）；簽章沒變且便箋投遞過就直接放行。",
    },
    "WIN-1": {
    ...
    "TITLE-2": {
        ...
        "why": "這一輪宣告了任務範圍，但標題還是佔位名（從未命名／平台預設代號／舊格式待命名／【待】【閒置】）"
               "，或還掛著上一件任務的名字·BLOCK 擋下一步操作",
        "tip": "TITLE-1（純文件規則）100% 靠模型自律，同一天就漏做一次——`session_title.py` "
               ...
               "本身，只擋「不改名就做別的事」。"
               "2026-09-07 真對話實測發現最大的漏洞不在佔位名：桌面版 /clear 產生的新 session "
               "會沿用上一則的 custom-title，判成「已命名」⇒ 換則接手新任務時閘門一次都不開火，"
               "而那正是本 repo 最主要的工作流。因此補上第二個判準「換題沒改名」——"
               "**只比名稱段**：進度／階段往前跳不算，措辭伸縮（一方是另一方的子字串）不算，"
               "user 自己取的標題（不帶【】標記）一律不動。詳見 SESSION_TITLE_HOOK_PLAN.md。",
    },
    "LEARN-1": {
        "badge": "9/06 新·shadow", "on": "<b>Stop</b>（不掛 SubagentStop）",
        "why": "這一輪自我宣告階段是 Execute／Design，卻沒問過「技術任務先問要不要學」也沒說「照做就好」",
        "tip": "輸出風格的這條規則跟 TITLE-1 一樣 100% 靠模型自律、零程式檢查——"
               "user 回報「很久沒有主動觸發」，但沒有紀錄能回答是真退化還是這段"
               "期間沒有技術任務。判準刻意只用**結構化的階段欄**當代理指標"
               "（不比對內文關鍵字——同一個道理：判準綁後果不綁字樣，關鍵字命中率"
               "不可預期），準不準留給 shadow 期的 applies／WARN 比例校準。"
               "一個 session 只記一次（不像 TITLE-2 用連續次數，這條要的是"
               "「這則有沒有問過」，不是「問了幾次都沒改」）。",
    },
    "DECL-2": {
        "badge": "9/06 新·shadow", "on": "<b>Stop</b>（不掛 SubagentStop）",
        "why": "這一輪有 Edit／Write 等動檔案的操作，但整段回覆找不到任何自我宣告",
        "tip": "補 DECL-1 的空白——DECL-1 只判「宣告存在時完不完整」，一輪整段"
               "零宣告時 applies 直接 False，連 WARN 都不會有。2026-09-06 稽核"
               "LEARN-1 有沒有真的在跑時，回頭核對本身這則對話才發現：23 個"
               "回合裡 20 個完全零宣告，DECL-1 一次都沒叫過。判準改用"
               "**這輪有沒有動檔案的 tool_use**（Edit／Write／MultiEdit／"
               "NotebookEdit，刻意不含 Bash——會混進大量純查證的假陽性）當代理"
               "指標，不猜「階段有沒有換」（沒宣告就無從得知階段換了沒，"
               "硬猜等於編造）。**沒有 session 去重**——跟 LEARN-1「一則只問"
               "一次」不同，這條要看的正是「這個 session 裡發生幾次」，"
               "去重會把最關鍵的頻率資訊抹掉。",
    },
}
