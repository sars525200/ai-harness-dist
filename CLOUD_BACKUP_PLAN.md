...
## 8. 棘輪自動放行：低風險形狀（設計中，2026-09-07·`.scratch/handoff/20260907-token-ratchet-autopass.md`）

> 為什麼要單獨一節：§3 的棘輪是逐顆 commit 卡人工判定的那一層。2026-09-07 11:01:12
> 那次推送就是活例子——`adae6629` 帶進 14 個陌生 token，棘輪全數擋下等人判，
> 這顆之後的 commit 到現在雲端都還沒有（`state/cloud_backup_failed.txt`）。
> 本節只處理「這一批陌生 token 裡，哪些形狀可以不必等人」，不動棘輪本身的判準邏輯。

### 8.1 現況（量出來的數字，2026-09-07 兩輪盤點）

- 敏感值（17 條規則左半邊）與已判無害的 baseline（約 7500 條，數字持續在動——
  盤點期間就被另一則對話追加過）做形狀比對：**L3 精確重疊 19.3%**，幾乎全部
  集中在 4 種形狀——純小寫字母、長度 7／8／9、無分隔（`a7`／`a8`／`a9`），
  以及使用者路徑結尾那段全大寫 4 碼。這 4 種形狀在 baseline 裡分別撞了
  605／524／324／1 次，**證據充分、必須永遠當人工判定**，不得自動放行。
- 除了這 4 種，其餘 13 條規則的形狀在 L3 這層要嘛**結構上摸不到**（私網 IP 是
  純數字、非 ASCII 公司名、短字串或無連字號的混合大小寫——這些值本來就不
  符合棘輪抽 token 的正則，寫不寫排除規則都沒差），要嘛在目前這批 baseline
  裡 0 撞擊。**0 撞擊不是「形狀安全」的證據，只是「現有樣本剛好沒有」**——
  跟 `a7`／`a8`／`a9` 那種有幾百次撞擊、證據紮實的排除不是同一等級。
- 依此重新定位：能拿到**實測零洩漏證據**、可以直接自動放行的只有**純十六進位
  字串**（長度 7／8／40／64，對到 git short hash／sha1／sha256 的常見長度）——
  覆蓋 baseline 的 **15.5%**（1171/7534，2026-09-07 量測），洩漏對照表跑過全部
  17 條規則左半邊，**零命中**。

### 8.2 目標

- 讓棘輪擋下的 fresh token 裡，形狀落在 hex 桶的部分**不必等人**，直接放行。
- **明講天花板**：這一輪只做 15.5%，**不是 80.7%**——後者是「排除已知會撞的
  形狀、假設其餘都安全」算出來的樂觀值，查證後發現站不住腳（見 8.1 第二點），
  不採用。**多數陌生 token 仍然要人工判定**，這是設計上的結論，不是還沒做完。
- `a7`／`a8`／`a9`、路徑結尾 `A4`，以及其餘所有非 hex 形狀，**明文釘死永遠人工**，
  寫進程式註解與本節，不留給下一個人自己猜。

### 8.3 做法（實作時發現比原設計簡單——寫下來以免下一個人重新發明）

- 原設計想在 `fresh` 之後另開一層。**實作時發現不必要**：`tools/push_cloud_backup.py:201`
  的 `tokens()` 早就在用 `HEX_RX` 把「長得像 commit hash」的字串整批擋在
  token 抽取階段，根本不會進到 `fresh` 候選——這正是 hex 桶自動放行要的效果，
  只是原本只收 git short hash／sha1 的長度（7–40），沒收 sha256（64）。
- 所以實際改動只有一行：`HEX_RX` 加一支 `|^[0-9a-f]{64}$`。
  **不新開一層、不動 `fresh` 的放行順序**——這樣改動面最小，也不會讓
  「這是 commit hash 本身」跟「這個字串形狀像雜湊、可放行」兩種判準理由
  混在同一段程式碼裡。
- 主控台輸出不必分兩段：hex 桶的 token 從一開始就不會出現在陌生 token
  清單裡，跟「這本來就不是陌生字串」的既有行為一致。

### 8.4 驗證（已跑，2026-09-07）

1. **洩漏回歸測試**：新增 `tests/test_push_cloud_backup_hex_bucket.py`，讀
   `.scratch/cloud-export/replace-rules.txt` 正本，逐條左半邊套用 `HEX_RX`，
   斷言零命中——不是手動跑一次算數，**掛進 `tests/run_hook_tests.py`
   自動跑**（正本規則檔不存在時印一行跳過說明，不假裝驗證過）。
2. **邊界／canary 測試**：同一支測試檔，正面驗長度 7/8/40/64 的合成 hex
   字串會被放行；負面驗長度 6／41／63（不支援的長度）與含非 hex 字元不會
   被誤放；再驗合成的「純小寫 7/8/9 字無分隔」帳號名形狀不落入 hex 桶
   （桶子邊界沒有畫歪）。
3. **先證明會紅**：故意把 `HEX_RX` 改回舊版（拿掉 64 那支），重跑這支測試——
   「長度 64 被放行」那條斷言確實轉紅；改回來後重新全過。這是這次改動
   唯一「會紅」的地方，不需要另外掛 `MUTATIONS`／`EQUIVALENT`（那套框架
   測的是規則資料檔被弄壞，跟這次改的是正則邊界不是同一種變異，硬塞
   進去只會讓兩套測試意圖混在一起）。
4. 三輪跑過：`tests/run_hook_tests.py`（2115/2115 全過，含新測試接線）、
   `tests/mutations/mutate_push_cloud_backup.py`（既有 9 條變異全過、正本
   6 個檔逐 byte 未變）、`tools/push_cloud_backup.py --check`（真實跑一次，
   除了棘輪本來就會擋的陌生 token 之外，其餘 14 項驗證照舊全過）。

### 8.5 狀態

**Execute 完成**（`tools/push_cloud_backup.py`、`tests/test_push_cloud_backup_hex_bucket.py`、
`tests/run_hook_tests.py`，尚未 commit）。`--check` 實跑時看到 23 個陌生 token
待人判——**這些不是本節的產物**，是別的 session 同時在動測試檔案帶進來的
（`AAA-1`／`agent-q1`／`s-direct` 這類形狀），本節新增的測試裡的合成字串
（`qzjmxpb`／`gggggggg`／`a1a1a1`）混在同一份清單裡，也還沒判定。
**觸碰棘輪基準線是另一個決定，本節不代勞**——那份檔案另一則對話正在同時寫，
不在這裡動它。

### 8.6 沒做的（本節範圍刻意不包含）

- `a7`／`a8`／`a9`、路徑結尾 `A4` 之外，**其餘結構上摸不到的規則類別**
  （私網 IP、非 ASCII 公司名、短字串）——這些本來就不是棘輪的問題，是
  §3 更早那兩層（清單／形狀）的涵蓋範圍，本節不重新設計那兩層。
- `mailmap.txt` 右側姓名本地部沒有規則涵蓋——沿用交接檔「未完成／等人
  點頭」第 2 項，跟本節無關，不在這裡處理。
- baseline 第 7392–7394 行三條含真實公司識別字、`floor-map-zone` 翻案窗口、
  文件與實作 9 處不符——全部沿用交接檔既有待辦，本節不重複列。
...
## 9. 手動 `--push` 一律走包裝器：結果檔不再說謊（2026-09-08·本則對話 6b34dc19）

> 為什麼要單獨一節：§6 把推送搬到背景時，寫結果檔（`state/cloud_backup_last.json`）的責任
> 落在包裝器 `cloud_backup_hook.py`，後端 `push_cloud_backup.py` 只推不寫。可是人手動
> 重推時走的是後端——2026-09-07 與 09-08 各踩一次：棘輪擋下 → 人審完 token → 手動
> `--push` 推成功 → 結果檔仍停在「失敗」→ 開工檢查 [4] 照樣報 `[!!] 結果檔在說謊`。
> 備份是好的、守門永遠紅，紅久了人就不讀它了。

### 9.1 現況（改前）

- 兩道門：post-commit → 包裝器 `--run`（寫 state/）；人 → 後端 `--push`（不寫 state/）。
- 所有指路寫的都是後端那道門：開工檢查的兩句提示、本計畫書 §2／§6 的指令、後端自己的 docstring。
  只有包裝器失敗標記裡的註解知道這件事，而那段註解是人推完之後才會看到的。
- 改文字＝要人記得走另一道門；上面兩次實踩證明記不住。

### 9.2 做法

- **後端 `--push` 沒帶標記就自己轉交包裝器 `--run --backend <自己>`**，結束碼原樣帶回。
  包裝器叫後端時帶環境變數 `CLOUD_BACKUP_VIA_HOOK=1`，後端看到標記才真的往下推——
  兩支同一個字串，漂掉會互踢到永遠，所以回歸網有對帳案例釘住。
- 帶 `--remote`／`--keep`／`--rules-dir` 的進階用法不轉交（包裝器不吃那些參數），
  但畫面講明「state/ 不會更新」。`--check` 與轉交無關。
- 指路文字一個都不用改：`--push` 現在就是會留紀錄的那道門。

### 9.3 驗證（2026-09-08 實跑）

- `tests/test_push_cloud_backup_manual_push.py` 13 案例：把後端複製到暫存目錄、旁邊放假包裝器
  （只記錄怎麼被叫、回 7），證明沒標記轉交／帶標記不轉交／進階參數不轉交／結束碼帶回／兩支標記名對帳。
- `tests/test_cloud_backup_hook.py` +1 案例（57/57）：包裝器叫後端時帶 `via=1`。
- 變異 4 條全紅：拿掉轉交段、包裝器不帶標記、標記名漂掉、轉交後不帶結束碼。
- 真機兩輪：第一輪走 `--push` → 轉交 → 棘輪擋下 cce7876 帶進的 37 個 token → **結果檔如實寫失敗**
  （改前這裡會停在舊紀錄）；判完 37 個無害、更新規則檔副本、再走 `--push` → ok=true、
  雲端 tip a89549a4、失敗標記自動刪除，開工檢查 [4] 轉 `[OK]`。

### 9.4 沒做的

- 棘輪本身每次別的 session 一 commit 就可能再紅（本日三輪共 100 個 token 全是測試假資料與
  程式名詞）——那是 §8 的題目（低風險形狀自動放行），這一節不碰判準。
- 沒動包裝器的鎖／待推／補推邏輯，只加一個環境變數。
