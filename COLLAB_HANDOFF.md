...
### 4. 角色數量兩邊不等（**已定案，2026-09-04 補記**）

`agents/` 有一個 `locator.md`（唯讀定位員），`cursor-agents/` 沒有對應檔——**這是刻意的，不是漏掉**。

⇒ 自己量：`ls agents/ cursor-agents/` 比清單。

⇒ **決定（`.scratch/handoff/archive/20260825-roles-cursor.md` 2026-08-25 定案）**：
Cursor 的探勘／查詢站**用內建 Task `explore`**，同時頂 Claude 這邊 `Explore`
（一次性探索）與 `locator`（唯讀定位）兩個角色，**不自建查詢員**。
理由是 Cursor 的內建 `explore` 子代理本來就唯讀、模型可指定 Composer 2.5，
沒有必要再疊一份格式不同、平台不吃 hook 的 `.md` 檔去模擬同一件事。
