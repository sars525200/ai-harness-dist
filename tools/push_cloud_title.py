...
# 續命的實作 2026-09-03 搬進 `hooks/session_title.py`（`_access_token()` 旁邊），
# 讓封存改名那條路也用得到同一份。這裡保留名字是為了**不改呼叫端與測試**——
# 兩份實作遲早分岔，而分岔的症狀是「某一條路安靜地不會續命」。
_RENEW_GUARD = T._RENEW_GUARD
_RENEW_TIMEOUT = T._RENEW_TIMEOUT
_renew_token = T._renew_token
_token_or_renew = T._token_or_renew
