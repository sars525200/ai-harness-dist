...
def _memory_offline_line(repo: Path) -> None:
    ...
    ⛔ **2026-09-05：這台機器上這一段已經不會印任何東西。** user 裁定記憶不跨機器
    延續 ⇒「整台機器掛掉要倒回記憶」這個情境不存在，而那是離線包唯一的存在理由。
    當天三層（獨立 repo／本機 bare 鏡像／NAS 離線包）**全部刪除**，設定的
    `memoryRepo` 與 `memoryOfflineBundle` 改成空字串，本函式因此在開頭就 return。
    **以下敘述保留為「這條線設計成什麼樣」的說明**，不是這台機器的現況；
    別的部門要用時填回那兩格即可。經過見 `MEMORY_RESTORE_PLAN.md`。
