...
- 2026-08-22 **v7（僅狀態欄，於 IGNORE 區內補記）**：user 委任「直接幫我規劃」後，剩餘分岔一次定案：
  - **分岔 0 改判：只有 M 級走 wayfinder**（原 S＋M 撤回）。理由是實測——規模欄 475 段宣告
    51.4% 未填或待定（路由要吃的輸入一半不存在）；S＋M＝74% session 要先開 map 且靠人記得打
    `/wayfinder`；M 級判準是事實性的、只佔 29%、本來就計畫書先行。
  - **分岔 1＝C＋D**（user 親選）：PR-1 對 map 存在即檢查＋Decisions-so-far 包 IGNORE＋連帶解 K14。
  - **分岔 2＝B**（M 級收斂後回寫 `*_PLAN.md` 摘要）＋票 09 的 shadow 修法（已完成，harness
    commit `09bfe8d`）。六個下游逐一交代見票 03；K5（Phase 進度圖）前提在 M-only＋B 下消失。
  - **分岔 3＝A（維持現況）**：`.scratch/` 進版控——事實上已經如此（票都已 commit），
    M-only 之下量小，接受外部 `git add -A` 的共存成本。
  - 決議全文與逐下游對照在 `d:/IT-department/.scratch/wayfinder-planning-layer/issues/02、03`；
    票 07 隨前提消失關 wontfix；剩餘可做＝票 04（tracer bullet，PR-1 實作）→ 05 → 06 → 08 → 09 key。
...
> 慣例全文＝`d:/IT-department/docs/agents/issue-tracker.md`「回寫摘要」節。
> 票本體在 `d:/IT-department/.scratch/wayfinder-planning-layer/issues/`；
> **這張表是給看板產生器讀的鏡像**，票才是真相，兩邊不一致以票為準。
> 開票就補一列、關票就把狀態格改成 `✅ 已完成`。

| 狀態 | 項目 | 說明 |
|---|---|---|
| ✅ 已完成 | 票 06 看板待辦鏈讀得到決策票 | 回寫慣例；本區塊即其產出 |
| ✅ 已完成 | 票 07 Phase 進度圖不再靜默停更 | 收斂時補 §3 條目；掛 06 之後 |
| ✅ 已完成 | 票 08 稽核角色對象涵蓋決策票 | 紅燈＝植入漂移必須被回報 |
| ✅ 已完成 | 票 09 遵循度軌跡 key 換工作單元 | shadow＋key 換皆已落地（`1735e6e`） |
| ✅ 已完成 | 票 05 驗證方式空著就擋 | /design-spec 步驟 4 的等價物 |
| ✅ 已完成 | 票 10 PR-1 閘門 hardening | fail-open 留痕／併行雙 hash |
| ⏳ 待做 | 票 11 覆核 R2 機制缺口（九項） | PR-1 五項＋量測三項＋看板鏈專案側；**修的是覆核機制自己，改完須重跑覆核** |
<!-- REVIEW_SCOPE_IGNORE_END -->
...
> **⚠ 2026-08-27：這張表已被機械產生的真相表取代，只留作沿革描述。**
> 真相在 `skills/_meta/LOCAL_EDITS.md`，產生器 `tools/local_edits.py`（權威是 `git diff`）。
> **不要再拿這張表當復原依據**——實測它已漏 4/8 且行號腐爛，而**漏掉的地方長得跟已驗證一模一樣**。
> 機械表額外講出這張表講不出來的三件事：①每支的基準是**真 upstream** 還是只是**匯入 commit**
> （後者會少算匯入當下就帶的在地改動，那是「量不到」不是「0」）②有分歧卻整檔零 marker 的檔
> ③有 marker 卻零分歧的過期標記。後兩項只在基準是真 upstream 時才算得出來，算不出來就標 `n/a`。
