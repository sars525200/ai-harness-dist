...
| **PR-1（Stop：計畫書審查 marker）** | ✅ 2026-07-28 實作＋8 fixture＋回歸網有效性驗證＋端到端 dry-run（真實 session 被 exit 2 擋回）。**enforce（2026-08-07）**；便箋 hash 相符時降 WARN（8/23）。設計與實作偏離見 `STOP_HOOK_MARKER_PLAN.md` §4.2 |
...
## 附錄：Meta-Skill（EDD 圖上的 04 自我迭代）—— 刻意未做

2026-08-23 從 `TODOS.md` 移進來當背景，**不是待辦**：它從 7 月掛在待辦表上、
每次盤點都要重讀一次，但它的觸發條件不是「排到它」而是「地基穩了」。

**為什麼不做**：地基沒鋪完就談迭代，迭代的是沙。

**什麼時候重評**：等四層穩定之後。評估時**先答一個問題** ——
「現在哪一層的規則還在變？」還在變的那一層就是地基還沒乾。
